...
sub run {
    ...
    my $auto_update = App::Its::Potracheno::Update->new(
        %{ $config->{update} } );
    $model = App::Its::Potracheno::Model->new( config => $config );

    # set global vars TODO make better
    $HELP = "$root/help";
    $new_banned = $model->get_config("security", "members_moderated");
    $forgot_ttl = $model->get_config("security", "reset_ttl") || 24*60*60;
    $search_limit = $model->get_config( search => "limit" ) || $search_limit;
