...
Returns sql schema as text. This is a static method.
...
sub find_user {
    ...
    if (defined( my $like = $opt{like}) ) {
        push @criteria, "(name LIKE ? OR email LIKE ?)";
        $like =~ s/%/_/g;
        push @args, "%$like%", "%$like%";
    };
    if (defined $opt{admin}) {
        push @criteria, "admin = ".($opt{admin} ? 1 : 0);
    };
    if (defined $opt{banned}) {
        push @criteria, "admin = ".($opt{banned} ? 1 : 0);
    };
    # TODO and merge other search methods in here

    $self->my_croak( "No search criteria, required one of 'like', 'admin', or 'banned'" )
        unless @criteria;

    my $sth = $self->_prepare( "SELECT * FROM user WHERE". join " AND ", @criteria);
    $sth->execute(@args);
...
=item * pass - if not given, will be blocked from logging in.
Password is stored encrypted, with individual salt.

=item * email - email address (not required as of current).
...
my $sql_user_ins = <<'SQL';
INSERT INTO user(name,password,banned) VALUES (?,'',1);
SQL

sub add_user {
    my ($self, %opt) = @_;

    $self->_croak("User name  is required")
        unless $opt{name};
    ...
    eval {
        $sth->execute( $opt{name} );
    };
    return if ($@ =~ /unique/);
    die $@ if $@; # rethrow

    # Then save real data
    $opt{user_id} = $self->dbh->last_insert_id("", "", "user", "user_id");
    $opt{banned} ||= 0;
...
sub save_user {
    ...
    defined $data->{$_} and $new{$_} = $data->{$_} ? 1 : 0 for qw(admin banned);
    defined $data->{$_} and $new{$_} = $data->{$_} for qw(email);
